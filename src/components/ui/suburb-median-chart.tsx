...
export function SuburbMedianChart({ data, suburbName, className }: SuburbMedianChartProps) {
  ...
  // Transform the data to ensure proper format with colors
  const chartData = validData.map((item, index) => ({
    year: item.year.toString(),
    value: item.value,
    fill: `var(--chart-${(index % 5) + 1})`, // Cycle through chart-1 to chart-5 colors
  }))
    ...
    <Card className={className}>
      ...
      <CardFooter className="flex-col items-start gap-2 text-sm">
        ...
        <div className="leading-none text-muted-foreground">
          Showing median property values over time
        </div>
      </CardFooter>
    </Card>
  )
